...
public class ReplaySerialize
{
    ...
    {
        return new Replay( null, r.TicksUTF, r.Actions, r.StartTime, r.EndTime, r.Indent );
    }
    public static Replay DictToReplay( Dictionary<string, object> d )
    {
        //Log.Info(d["Actions"]);
        ...
        try
        {
            indent = JsonSerializer.Deserialize<string>( (JsonElement)d["Indent"] );
        }
        catch ( Exception e )
        {
            indent = "move.errormap";
            Sng.ELog( e, "err" );
        }
        return new Replay(
                new List<Tick>(),
    ...
    public static Replay JsonToReplay( string json )
    {
        return JsonSerializer.Deserialize<Replay>( json );
    }
