...
public class ViewReplay
{
	...
	void NextTick()
	{
		...
		CurrentTick = newTick;
		SPIN.ApplySpinSpeed( (Replay.Ticks[previousTick].Spin + (Replay.Ticks[nextTick].Spin - Replay.Ticks[previousTick].Spin) * factor) * ReplaySpeed );
	...
	public void JumpToTick( int tick )
	{
		...
		SPIN.ApplySpinSpeed( Replay.Ticks[tick].Spin );
